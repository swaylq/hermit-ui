...
- [ ] **`packages/tmux-driver`** — new package. Exports: `ensureSession(sessionId, cwd, claudeArgs)`, `sendKeys(sessionId, text)`, `kill(sessionId)`, `getClaudeSessionUuid(sessionId)`, `watchTranscript(uuid, onMessage)`. Implementation cribbed from happy's `utils/tmux.ts` but no encryption/network — local only.
- [ ] **JSONL watcher** — chokidar tail of `~/.claude/projects/<encoded-cwd>/<uuid>.jsonl` (parse line-by-line, dedup by msg uuid, emit content blocks). One watcher per session.
- [ ] **rewrite `apps/gateway/src/chat-runner.ts`** — chatTick now calls `ensureSession()` once per ChatSession, `sendKeys` per user message, watcher streams assistant messages back to `/api/sync/chat-message`. Remove the entire `spawn(CLAUDE_BIN, ['--print', '-p', ...])` block. Cancellation = `sendKeys(s, 'Escape')` (claude's interrupt) instead of SIGTERM.
- [ ] **multi-session per agent test** — start 2 ChatSessions against `agents/alpha`, send different prompts simultaneously, verify both tmux panes alive + transcripts isolated + DB rows correct.
