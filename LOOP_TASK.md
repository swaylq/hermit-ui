...
- [ ] **rewrite `apps/gateway/src/chat-runner.ts`** — chatTick now calls `ensureSession()` once per ChatSession, `sendKeys` per user message, watcher streams assistant messages back to `/api/sync/chat-message`. Remove the entire `spawn(CLAUDE_BIN, ['--print', '-p', ...])` block. Cancellation = `sendInterrupt(s)` (claude's Escape) instead of SIGTERM.
