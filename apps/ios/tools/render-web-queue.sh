...
# tools/fixtures/queue-bar.json, so the two PNGs are one table drawn twice.
...
for scheme in dark light; do
  ( cd "$REPO/apps/dashboard" && HERMIT_QUEUE_FIXTURE="$IOS/tools/fixtures/queue-bar.json" \
      "$NM/.bin/tsx" scripts/render-queue-bar.tsx "$TMP/queue-$scheme.html" "$scheme" ) \
  ...
  if [ -n "${HERMIT_MEASURE:-}" ]; then
    python3 -c 'import sys;p=sys.argv[1];s=open(p,encoding="utf8").read();open(p,"w",encoding="utf8").write(s.replace("</body>",sys.argv[2]+"</body>"))' \
      "$TMP/queue-$scheme.html" "$PROBE"
    "$CHROME" --headless=new --disable-gpu --no-first-run --no-default-browser-check \
      --user-data-dir="$TMP/profile-$scheme" --hide-scrollbars \
      --force-device-scale-factor=3 --window-size=390,"$HEIGHT" \
      --virtual-time-budget=2000 --dump-dom "file://$TMP/queue-$scheme.html" \
  ...
  shot="$OUT/web-queue-strip-$scheme.png"
  rm -f "$shot"
  "$CHROME" --headless=new --disable-gpu --no-first-run --no-default-browser-check \
    --user-data-dir="$TMP/profile-$scheme" --hide-scrollbars \
    --force-device-scale-factor=3 --window-size=390,"$HEIGHT" \
    --virtual-time-budget=1500 --screenshot="$shot" \
    "file://$TMP/queue-$scheme.html" >/dev/null 2>&1 &
  chrome_pid=$!
  for _ in $(seq 1 100); do
