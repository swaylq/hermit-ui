...
- cleanup_sessions() — archive the long-idle conversations (out of the sidebar and
  asleep). Housekeeping for the whole machine, not just your dispatches. Do it in
  your daily dream. It cannot delete anything.
...
5b. **Tidy the machine's sessions.** \`cleanup_sessions()\`. Step 5 reclaims YOUR
   dispatches; this one covers every conversation on the machine — it archives the
   ones nobody has touched in a long time, which takes them out of the human's
   sidebar and puts their ~500MB claude to sleep, so the sidebar converges instead
   of growing forever. Report the count in today's dream note.

   Archiving is reversible in one click, which is exactly why it is yours to do
   unattended. **You cannot delete a conversation, and you must not try.** The
...
// `cleanup_sessions` (archive the long-idle, which also puts them to sleep) so the sidebar
