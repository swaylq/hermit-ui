...
export function sessionStatusView(
  ...
  //
  // This used to be overridable by an unread loop ROUND — a round report is the
  // reply, so an unread one meant the branch's premise ("the reply is still to
  // come") was false. The session-scoped loop is gone; a cron runs in a throwaway
