...
- **Every run must build.** That is the gate — not a test suite, and never one you wrote to
  ...
  that last report.
- **Test once, at the end, and only for big changes.** An iterating cron checks its finish
  line by USING the thing, on the run that thinks it is done, not by re-running a suite every
  round. A run that only moved a CSS rule, a constant or a string is finished when it builds
