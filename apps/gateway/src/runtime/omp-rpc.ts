...
import { providerEnv, visionEnv, readSecret } from './pi-credentials';
...
export class OmpRpcRuntime implements AgentRuntime {
  ...
  private async boot(session: RuntimeSession, emit: (item: SyncItem) => void): Promise<OmpHandle> {
    ...
    ensureOmpModelsYaml(cfg);
    const model = session.model ?? mode?.model ?? resolveDefaultModel(cfg);
    const provider = session.provider ?? cfg.provider;
    ...
    const args = [
      ...
      ...buildModeArgs(mode, { agentDirectory: session.agentDirectory, runtime: 'omp-rpc' }),
