...
- sway talks to you via the hermit-ui dashboard at `http://127.0.0.1:4101`.
- Messages arrive through the local gateway: it spawns a tmux pane running `claude`, sends `sway`'s text via `tmux send-keys`, and tails your JSONL transcript to stream your reply back to the browser.
...
- Cron tasks should report back to sway when done — push to the dashboard via `scripts/dashboard-push.sh "<message>" [type]`.
