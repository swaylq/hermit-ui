...
铺开时又抓到一个：`siblings` 原本只按「我自己那条会话的目录」回答，忽略调用方传进来的目录。生产里两者恰好相等，所以看不出来——但拿 A agent 的身份去查 B agent 的目录时，它会一脸认真地报出 A 的兄弟数。判定函数必须对**被问的那个目录**回答，不是对调用者碰巧属于谁回答；目录改成显式参数，hook 每次都传。
