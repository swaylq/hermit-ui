...
> **2026-08-30：本文描述的悬浮麦克风已经不存在了。** `voice-mic.tsx` / `fab-dock.tsx`
> 都已删除，`hermit:voice-mic-pos` 这个位置记忆也随之作废。现在的入口在输入框上：
> 按住输入框说话（松手发送 / 左滑取消 / 右滑留在框里改），或点输入框右边的麦克风
> 免提听写。权限请求的时机和降级逻辑仍然按本文写的来。
