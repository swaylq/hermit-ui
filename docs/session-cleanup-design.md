...
**原本是四档，「睡」在 2026-08-09 合进了归档。** 两个理由指向同一个方向：机器上本来就有一个
休眠器（idle-TTL reaper，`Machine.idleReapHours`，每 10 分钟一跳），清理再做一遍是重复；
而作为**给人看的概念**，「睡着」和「已归档」从来没有区分到值得占两行 UI、两个阈值。
归档现在顺手把进程睡掉 —— 这本来就是所有人默认它会做的事：一个已经离开侧栏的会话，
没有理由还占着 500MB。
