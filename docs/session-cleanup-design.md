...
**上传文件：跟着行一起删**（2026-09-01 补）。和 transcript 相反，这一类是**可证明属于我们的**：
composer 上传和 agent 的 `attach_file` / `attach_image` 走的是同一个 `/api/upload`，全都落在
`<UPLOAD_DIR>/<sessionId>/` —— 一个会话一个目录，路径本身就是索引，不需要另建一张表。
所以 `chat.ackPurged` 删完行之后紧接着 `deleteSessionUploads(ids)`（`server/session-uploads.ts`）。

三件事的顺序和位置都是有理由的：

- **在 dashboard 侧删，不在 gateway 侧。** 这些字节在 dashboard 的磁盘上，机器根本够不着 ——
  这也是为什么 transcript 由 gateway 删、上传文件由 dashboard 删。
- **先删行，再删文件。** 中间崩了的话，留下的是没人指着的孤儿字节（可扫、可补），
  而不是一个丢了图片的活会话。
- **id 必须长得像 cuid，且解析后必须落在 upload 根目录里面。** 这是个递归删除，
  `''` / `.` / `../..` 任意一个漏过去就会连 file-station 的在途文件一起端走。
  `session-uploads.test.ts` 整个文件几乎都在测**拒绝**。

补这一刀之前的账：2026-09-01 线上 upload 根目录 9.5 GB / 414 个目录，其中 65 个目录、347 MB
的会话行早就不存在了 —— 从来没有任何东西删过它们。
...
- **回收站视图**：复用 `trashed-agents.tsx` 的形状，列出 `trashedAt` 非空的会话 + 理由 + `恢复` / `立即抹除`。
  2026-09-01 分两步搬了家：先从清理卡底部那个 `Trash (78)` 文字链改成独立一张常驻卡，
  当天又整个提成 **Settings 里的一个页面** `/trash`（`components/recycle-bin-view.tsx`），
  System 页只留一个「N in the bin」链接过去。
  加上保留天数输入框、每行的上传体积和「还剩几天被删」倒计时 ——
  全站只有这一处东西在排队等着被永久删除，倒计时只有在不用去找它的时候才有用。
  页面上**两个回收站都在**：会话（到期自动删）和 agent（`.hermit-trash`，只等人点头）。
  一个叫「回收站」的页面只显示删掉的东西的一半，是个坑。侧栏底部那个 agent 回收站保留，
  它是快捷入口、不是第二份实现 —— 两边读同一个 `agents.listTrashed`。
