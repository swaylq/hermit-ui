...
1. **前门必须退回网页的形状。** 网页 `/` → `/chat` 会恢复 `lastSessionId()` ——
   **打开就在上次那个会话里**，会话列表是盖在它上面的左抽屉（`app-sidebar.tsx:181`：
   ...
   sway 装上 build 7 第一次看见就提了。怎么退见「下一项」第 0 条。
...
**第 0 条：前门退回网页 —— 下一轮先做这条，排在上面第 1、2 条之前。**
（2026-09-05 17:30 重新提上来：sway 在等它落地，落地当天就会有人构建并 Wi-Fi 推到他手机上，
所以这条是唯一一条「做完立刻有人看得见」的。第 1、2 轮各自把它往后放了一次，别再放第三次。）
