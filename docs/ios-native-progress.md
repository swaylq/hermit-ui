...
- [x] 按住说话：`HOLD_MS=260`、`BAIL_PX=10`、`SLIDE_PX=64`，三个区是
      发送 / 左划取消 / 右划落回输入框（**没有「转文字」这个区**）—— 第 9 轮。
      `Hermit/HoldCore.swift`（阈值 + 区判定 + 那张微信圆的几何 + 两个命中框）
      配 `tools/hold-fixture.sh`，`Hermit/HoldToTalkView.swift` 画那一屏，
      `tools/hold-compare.sh` 和网页逐像素比 **1.65%**
- [x] 听写：`/api/asr/<sid>` WebSocket，鉴权走 `Sec-WebSocket-Protocol: hermit-key.<token>`
      （原生可以直接设 header，但服务端认的是 subprotocol）。上行 16kHz 单声道 PCM16
      二进制帧；下行 `partial` 整段重写、`final`/`polished` **乱序到达，按 segId 定位**
      —— 第 9 轮。`VoiceCapture`（AVAudioEngine → `AVAudioConverter` 重采样）+
      `AsrSocket` + `DictationRun`，纯函数在 `DictationCore.swift`。
      **音频那一段这台 Mac 验不了**（见下一条）
- [ ] **听写的音频链路没有任何证据** —— 这台 Mac **没有任何音频输入设备**
      （`system_profiler SPAudioDataType` 只有输出），模拟器因此没有麦克风，
      `AVAudioEngine` 起不来。socket 那半有 `tools/asr-fixture.sh`（真的 WebSocket，
      不需要麦克风）、手势和那一屏有模拟器用例和像素比对，**从麦克风到 PCM16 这一段
      只过了类型检查**。要么给这台机器接个输入设备，要么下一轮写一个把
      `VoiceCapture` 的转换器喂已知波形的宿主机夹具
- [ ] **听写的降级路径**：网页拿不到 socket（没配 key、没有模型、连接中断）时
      **不失败而是退化**——整段录下来，松手时走 `/api/transcribe` 批量转，
      words 晚一点到而已；原生今天是「说清楚为什么，然后结束」。
      还有收尾的整段校正（`worthRefining` 已移植并进了夹具表，但还没人调用它）
- [ ] **听写气泡的 8 行裁在头上还是尾上**：网页 `hold-to-talk.tsx` 那段注释说
      「尾巴是刚到的，所以框里显示的是末尾」，而它写的 CSS 是 `-webkit-line-clamp: 8`
      —— **裁的是尾巴，显示的是开头**。原生按网页**实际画的**移植（同一个 clamp），
      两边像素对得上。这是网页自己的一个小 bug：说的话超过八行之后，
      新到的词就再也看不见了。**要 sway 定：改网页去对齐它的注释，还是两边都保持现状**
...
微信那两条弧的注释是网页自己写的：**环和圆头共用一个透明度，分开上色到它们相交的地方就露馅
—— 两个 14% 的白叠起来合成 26%，接缝处会出现一个亮盘压在带子上**。Swift 版把
`.opacity(0.14)` 加在包着两者的 `ZStack` 上，看起来就是「一组」，**SwiftUI 却是分别淡化每个
子视图**，于是原原本本地复现了那段注释警告的东西。热力图上是两个半圆亮红，
`rowdiff` 量出来相交处 66 对网页的 36 —— `1-(1-0.14)²=0.2604`，正好。

同一小时里第二次：气泡的 `.shadow`。CSS 的 `box-shadow` 是**盒子**的属性，SwiftUI 的
`.shadow` 也是逐叶生效 —— 每个字都在白气泡上投了自己的影子。屏幕上看不见
（这里影子是黑的、背景也是黑的），`pixel-probe` 一眼：气泡内部从上到下 253 → 245 渐变，
越靠近字越暗。两处都是 `.compositingGroup()`，**都不是装饰**。

### 黑底上比像素，看不见「蒙层有多黑」（第 9 轮）

`hold-compare.sh` 第一版两边都画在纯黑上 —— 理由听起来很对：`bg-black/70` 后面是什么
不是这个比对要管的事，画一整条时间线在后面等于把时间线比两遍。结果是
**黑蒙层盖在黑底上，两边都是黑**，这一项彻底比不出来，报了个 1.65% 却对蒙层一无所知。
改成两边都画在 app 自己的**浅色背景**上之后：蒙层本身两边都是 94（Chrome 和 SwiftUI
在这件事上意见一致，都不是教科书上的 76 —— 两个引擎都不在 sRGB 里合成），
但**气泡的阴影立刻红了一大圈**：CSS 的 `box-shadow: 0 18px 50px -12px` 有 spread，
SwiftUI 的 `.shadow(radius:)` 没有，少了那个 −12 就是每边宽 12pt。
改成手画（把气泡的形状 inset 12、位移 18、blur 25 垫在后面）之后，
下沿的衰减和 Chrome **逐行同为 75/76/77/78**。

**这条的教训不是「阴影要手画」，是「比对的背景要选那个能让差异现形的」。**

### headless Chrome 的 viewport 不是 window，而且两种模式还不一样（第 9 轮）

`hold-to-talk` 的正文区是 `pb-[20vh]`，所以比对只有在浏览器视口**正好等于表里那块屏**时才成立。
它不等于 `--window-size`：headless 给自己留了约 87px。而且两种模式的答案相反 ——
`--screenshot` 会先把视口撑到整页再截，所以 `--window-size $W,$H` 给的 `innerHeight` 就是 H；
`--dump-dom` 不会，得要 `H + 87`。同一个脚本里两条命令用两个高度，
**是量出来的不是推出来的**（`tools/render-web-hold.sh` 里有一段注释和一次探测）。
用错的代价是整屏的字往下挪 17pt，而热力图会把它算在移植这边头上。

### `sips --cropOffset` 是从图片中心量的，不是从左上角（第 9 轮，文档里早就写着）

`pixel-probe.swift` 开头那段警告说过「两次错误结论是这么来的」，这一轮又来了一次：
把 2817 高的截图「裁掉底部」裁成 2556，实际是从中心裁的，**上面少了 130 行**，
于是每个元素都错位 43pt，看图会得出「原生整体偏低」这个完全错误的结论。
现在有 `tools/png-crop.swift`（CGImage 的原点就在左上角），以后裁图用它。

### 这台 Mac 没有任何音频输入设备（第 9 轮）

`system_profiler SPAudioDataType` 只列出输出。模拟器用宿主机的输入设备，
所以**模拟器上也没有麦克风**，`AVAudioEngine` 的 `inputNode` 起不来。
后果不是「测试不好写」，是**听写的音频链路在这台机器上拿不到任何证据**。
能做的两件都做了：socket 那半用 `tools/asr-fixture.sh`（自己手写的 WebSocket 服务端，
不需要麦克风）、手势和那一屏用模拟器用例和像素比对。**没做到的那件已经写成清单上的一条**，
没有藏在散文里。

### 夹具里的 id 不能正好等于数组下标（第 9 轮）

`asr-fixture` 第一版服务端发的 `segId` 是 0 和 1。反证「把按 id 找改成按下标找」时
**它没红** —— 因为 0 和 1 恰好就是下标。改成 4 和 9 之后立刻红两条。
共享表那边同一条反证是红的（它有一个「id 不必从 0 开始也不必连续」的用例），
所以两张表都要有这条，不能指望另一张替自己看着。

同一轮的第二次：`worthRefining` 用 `utf16.count` 还是 `count`，
表里**每一个 CJK 和 Latin 字符串两者都相等**，所以反证不红。补了两条 emoji 的用例
（八个 emoji 是 16 个 UTF-16 单元、8 个 Character），才第一次红。
**没人会这么说话，这不是重点** —— 重点是这条阈值是照着 JS 的 `.length` 定的。
