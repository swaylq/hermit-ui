...
> **2026-08-30：入口部分已被取代，管线部分仍然有效。**
> 下面写的浮动麦克风（`voice-mic.tsx`）、听写状态条（`dictation-bar.tsx`）、
> 波形（`voice-wave.tsx`）三个文件都删掉了。现在的入口是：在输入框上按住说话
> （松手发送 / 左滑取消 / 右滑留在框里改，见 `hold-to-talk.tsx`），或点输入框
> 右边的麦克风免提听写（听写中该图标亮起，再点一下结束）。
> ASR 管线、断句纠错、通读整理、降级成录音这些本文其余部分都没变。

**Goal:** 把「长按录完 → 松手 → 等 1.5s → 整段文字掉进输入框」的语音输入，换成**边说边出字**：说话的同时文字实时上屏，每说完一句（VAD 断句）后台悄悄跑一遍 AI 纠错，把那一句就地替换成准确版本 —— 用户全程不等待，看到的是文字自己长出来、自己改对。
...
**这套已经删掉了**，不是并存的第二种模式：每一次按压都是实时听写。整段那条链路（`/api/transcribe`、定稿提示词、上下文）原封不动留着，但它现在只是**降级路径**——用户永远不会主动选它，见文末「降级」。
...
**麦克风在 `pointerdown` 就开始录**，还不知道这是点还是按 —— 等判断出来再开麦会吃掉第一个字。这一按到底算什么，是**松手之后**才定的：

| 判定 | 含义 |
|---|---|
| 松手 < **400ms** | 是「点一下」→ 这次听写**继续跑**，变成免提；再点一下结束 |
...
**听写期间 FAB 必须留在屏幕上**：一个按钮如果在按压中途卸载/消失，它的 pointerup 就永远不会送达，按住说话就结束不了。

**但结束不能只靠 FAB。** FAB 的位置是按 `window.innerHeight` 夹的，而 iOS 上键盘弹出时 `window.innerHeight` **不变**——停在底部（默认位置）的 FAB 就正好躲到键盘后面，看不见也点不到。这是手机端「有时候看不到 stop 键」的真实原因。听写条在 composer 自己的栈里、跟着一起动，所以 **✓ 和 ✕ 都放在条上**，位置在**最左边**：FAB 默认在右下角，条上唯一会被它压住的只剩计时，压住计时只是不好看，压住出口就是 bug。

麦克风本身仍然可以结束（松手，或再点一下），条上的 ✓ 是那个总在的兜底。
...
ASR 是**一坨一坨**给的：400ms 什么都没有，然后一次蹦四个字。直接写进去就是抽搐。中间夹一层 `lib/typewriter.ts`，按 28ms 一帧、约 36 字/秒吐出来（ASR 平均才 ~10 字/秒，追得绰绰有余），于是文字是「到达」而不是「出现」。
...
|---|---|
| **追加**（常态） | 一帧一个字往外吐 |
| **短回退**（ASR 自我改口，`发 red hot` → `把Red Hole`） | 错的尾巴**一帧删完**，然后重打 |
| **长回退**（纠错落在你已经说完的上一句，后面还跟着第二句） | **直接跳**到位 |

短回退最早是一个字一个字退的，看着像渲染 bug 而不像改口 —— 半删状态是「…PADDY重重」这种不成词的东西。一帧删完再重打，既保留了「它改主意了」的观感，又没有中间的怪状态。长回退跳而不退，是因为为了改上游一个词而把后面整句倒着退回去，churn 太大而收益是零。

阈值按**离末尾多远**算（`MAX_REWIND = 12`），不是按 diff 大小：纠错刚好落在上一句、后面才起了个头时（实测 `…上的PADDY重启一下。然后` → `…上的Caddy重启一下。然后`）仍然是「回退 + 重打」，看得见它在改。
...
三种手势各跑一遍，每 400ms 读一次真实 textarea 的值：

**A. 点一下 → 免提**（松手后继续说，再点一下结束）
...
t=1.2s  "帮我把这ap"                                   ← 逐字揭示
t=1.6s  "帮我把这apan"
t=2.4s  "帮我把JAPAN DEV上的"
t=4.0s  "帮我把JAPANDEVE上的PADI重启一下。"             ← sentence_end
t=4.4s  "帮我把 japan-dev 上的 PADI 重启一下。然"        ← 纠错就地替换，第二句已在后面
final   "帮我把 japan-dev 上的 PADI 重启一下。然后检查一下证书，还有。"
```

**B. 按住说话 → 松手结束**：按住期间条在、字在长；松手后收尾，条消失，草稿保留。

**C. 按住后上滑 → 取消**：草稿回到听写开始前的 `先打了几个字`，**一个多余的空格都没留**。
...
整段模式已经不是用户能选的东西了，所以这里必须兜住所有 socket 服务不了的人。**降级不打断这次听写**：麦克风照录，只是从「边说边出字」变成「说完一起出」，条上写明「实时转写不可用 · 正在录音，松开后转写」。

- **socket 中途挂了**（WS 断、限流、Caddy 掐连接）：采集层一直留着**上一句断句之后**的音频（每次 `final` 清空，通常几秒），所以从断点继续录是接得上的。松手时整段 POST 给 `/api/transcribe`，词接在已确认的句子后面。一个字都不丢。
- **当时屏幕上那半句 partial 要撤掉**：它对应的音频马上要被重新转写一遍，留着就会出现两遍。
- **没有 `DASHSCOPE_API_KEY`**：服务端明说 `not configured`，本次直接录音，并且**这一页之后都不再开 socket**——那个答案不会变，没必要每次都去被拒一遍。
- **scoped agent 分享 token**：socket 只认 machine key，401，同上路径。
