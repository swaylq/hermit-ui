...
**听写期间 FAB 必须留在屏幕上**：一个按钮如果在按压中途卸载/消失，它的 pointerup 就永远不会送达，按住说话就结束不了。所以避让重叠的是**听写条**——它唯一的按钮（取消）挪到了最左边，而 FAB 默认在右下角；条最右边只剩计时，被压住也只是不好看，不会误触。结束永远是麦克风本身：松手，或者再点一下。
