...
- [ ] **P0-1 · Index `ChatSession(machineId, lastMessageAt)`.** One additive migration; makes the `ORDER BY lastMessageAt DESC` in `notifications.counts` (S1), `notifications.feed` (S2), and `chat.listSessions` (S4) index-backed instead of a full-set filesort. *Test:* migration applies on VPS; `notifications.counts` + `listSessions` return identical data; `curl` timing before/after (report the delta).
